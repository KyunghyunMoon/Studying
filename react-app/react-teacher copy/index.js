...
app.post('/menulist', (req, res) => {
  ...
  const sqlQuery =
    'SELECT menu_storeId, menu_name, menu_price, menu_pictureUrl FROM menu_tbl LIMIT ?, ?;';
...
app.post('/menuinsert', (req, res) => {
  ...
  const sqlQuery =
    'INSERT INTO menu_tbl (menu_storeId, menu_name, menu_price, menu_pictureUrl) VALUES (?, ?, ?, ?);';
  // (?, ?, ?) : [writer, title, content]를 파라미터 값으로 받아온다는 의미
  db.query(sqlQuery, [menu_storeId, menu_name, menu_price, menu_pictureUrl], (err, result) => {
    // res.send(result);
    res.send('확인');
