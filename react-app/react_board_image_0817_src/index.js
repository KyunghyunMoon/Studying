...
app.use("/uploads", express.static("uploads"));  //없으면 이미지 파일 표시 안 됨.
...
app.post("/insert", upload.single("image"), (req, res) => {
  ...
  db.query(
    sqlQuery,
    [writer, title, content, req.file.filename],  //파라메타 req추가. 업로드 할 파일 이름
    (err, result) => {
      res.send(result);
    }
  );
...
app.post("/detail", (req, res) => {
  ...
  const sqlQuery =
    "SELECT BOARD_NUM, BOARD_WRITER, BOARD_TITLE, BOARD_CONTENT, DATE_FORMAT(BOARD_DATE, '%Y-%m-%d') AS BOARD_DATE, BOARD_IMAGE FROM BOARD_TBL where BOARD_NUM = ?;"; //이미지 수정
