...
const sqlQuery = "select count(*) as 'cnt' from store_member where store_id=? and password=?;";
db.query(sqlQuery, [store_id, password], (err, result) => {
  res.send(result);
});
});
//   if(result[0].cnt ===1) {
...
app.post("/store_member", (req, res) => {
  console.log("/store_member", req.body);
  var store_id = req.body.store_id;
  var password = req.body.password;
  ...
  const sqlQuery = "insert into store_member values (?,?,?,?,?,?,?,?);";
  db.query(sqlQuery, [store_id, password, passwordcheck, email, name, address, phone, s_name], (err, result) => {
    res.send(result);
  });
});
